...
test('CRA reporting is disabled by default and does not alter Compliance read API configuration', () => {
  ...
  assert.equal(DEFAULT_CONFIG.craReporting.assessmentId, '');
...
test('CRA reporting config loads additively and reuses Compliance tenant/project identity', async () => {
  ...
  await writeFile(join(root, '.sentrycode', 'config.json'), JSON.stringify({
    ...
    craReporting: { enabled: true, endpoint: 'https://cra.example/api/v1/invoke', assessmentId: 'cra-assessment-1', tokenEnv: 'CRA_TOKEN', timeoutMs: 9000, maxAttempts: 7, retryDelayMs: 12000, severities: ['critical'], findingTypes: ['vulnerability', 'sast'] }
  }));
  ...
  assert.equal(config.craReporting.endpoint, 'https://cra.example/api/v1/invoke');
  assert.equal(config.craReporting.assessmentId, 'cra-assessment-1');
