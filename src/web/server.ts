  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (url.pathname.startsWith('/api/')) {
        ...
        if(!isAdmin && options.token && supplied!==options.token){ json(res,401,{error:'unauthorized'}); return; }
        if(isAdmin && req.method==='GET' && options.token && supplied!==options.token && supplied!==options.adminToken){ json(res,401,{error:'unauthorized'}); return; }
        if(isAdmin && req.method!=='GET'){
          if(!options.adminToken){ json(res,503,{error:'administration_locked',message:'Set SENTRYCODE_UI_ADMIN_TOKEN to enable administrative writes.'}); return; }
          if(supplied!==options.adminToken){ json(res,401,{error:'admin_unauthorized'}); return; }
        }
        ...
        if(url.pathname==='/api/v1/admin/session'&&req.method==='POST'){ json(res,200,{authenticated:true,actor:actor(req)}); return; }
        ...
        if(url.pathname==='/api/v1/admin/repositories'&&req.method==='POST'){ json(res,201,await admin.registerRepository(await body(req),actor(req))); return; }
